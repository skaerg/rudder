...
//  }
  ...
  val nodeRepository: CoreNodeFactRepository = {
    ...
    val tenantRepository: TenantService = InMemoryTenantService.make(Nil).runNow
...
//    .setLevel(ch.qos.logback.classic.Level.TRACE)
  ...
  @Test def dateQueries(): Unit = {
    ...
    def q_compat(name: String, comp: String, day: Int, expects: Seq[NodeId]) = TestQuery(
      name,
      parser(s"""
          {  "select":"nodeAndPolicyServer", "where":[
            { "objectType":"node", "attribute":"inventoryDate", "comparator":"${comp}", "value":"${day}/05/2013" }
          ] }
          """.format(comp, day)).openOrThrowException("For tests"),
      expects
    )

    def q_iso(name: String, comp: String, day: Int, expects: Seq[NodeId]) = TestQuery(
      name,
      parser(s"""
          {  "select":"nodeAndPolicyServer", "where":[
    ...
    testQueries(
      ...
      query("q11", "gt", 14, s) :::
      query("q12", "notEq", 15, root +: s.filterNot(_ == s(0))) :::
      query("q13", "notEq", 14, root +: s) :::
      query("q14", "notEq", 16, root +: s) :::
      Nil,
